              ...
              return (
                <div key={`${row.accountId}:${row.id}`} style={{ transform: `translateY(${item.start}px)`, position: "absolute", top: 0, left: 0, width: "100%" }} ref={virtualizer.measureElement} data-index={item.index}>
                  <Row
                    ...
                    onHover={(e) => {
                      // Pointing at a row selects it, so the triage keys act on what is under the
                      // mouse without a click first. Only a real movement counts: rows sliding under
                      // a still pointer, while J and K run or the list scrolls, must not steal it.
