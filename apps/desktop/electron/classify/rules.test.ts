...
test("the six types: each one has a rule, and the ambiguous cases go to the model", () => {
  ...
  assert.equal(classifyByRules(input({ from: "daan@marblebar.example", subject: "Accepted: Arcforma Foundations | Session 2", headers: {}, hasCalendarPart: false, threadHasOutbound: false }), ctx).type, "calendar", "an acceptance from a person's own address, its own thread, no .ics in sight");
  assert.equal(classifyByRules(input({ from: "kate@kjtravel.example", subject: "Invitation: Kate <> Oliver @ Wed Aug 19, 2026 2:45pm - 3pm (PDT) (oliver@arcforma.ai)", headers: {}, hasCalendarPart: false, threadHasOutbound: false }), ctx).type, "calendar", "Google's when-suffix on an invitation from a person");
  assert.equal(classifyByRules(input({ from: "friend@example.com", subject: "Invitation to dinner?", headers: {}, hasCalendarPart: false, threadHasOutbound: false }), ctx).type, null, "an invitation word alone, from a person, is a conversation");
